...
class ActiveTrack(EcmEnv):
    ...
    def step(self, action: np.ndarray):
        ...
        print("activetrack step:", self._step)
    ...
    def compute_reward(self, achieved_goal: np.ndarray, desired_goal: np.ndarray, info):
        ...
        centroid, wz = achieved_goal, achieved_goal[-1]
        #print("compute reward active", centroid.shape, desired_goal.shape)
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        observation = np.concatenate([
            robot_state, np.array(in_view).astype(float).ravel(),
